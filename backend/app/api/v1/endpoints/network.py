...
from app.models.models import Agent, AgentConnection, AgentReputation, AgentStatusEnum, User
...
async def network_graph(db: DBSession):
    ...
    conn_q = select(AgentConnection)
    conn_result = await db.execute(conn_q)
    connections = conn_result.scalars().all()

    edges = [
        NetworkEdge(
            from_id=c.from_agent_id,
            to_id=c.to_agent_id,
    ...
    online = sum(1 for n in nodes if n.status == "active" or n.status == AgentStatusEnum.active)
...
async def network_stats(db: DBSession):
    ...
    online = (await db.execute(
        select(func.count()).select_from(Agent).where(Agent.status == AgentStatusEnum.active)
    )).scalar_one()
    return {
        "total_agents": total,
        "online_agents": online,
